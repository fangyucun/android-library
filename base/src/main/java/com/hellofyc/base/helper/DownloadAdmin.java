...
public class DownloadAdmin {
	...
	private static DownloadAdmin sInstance;
	private DownloadManager mDownloadManager;
	private OnDownloadListener mOnDownloadListener;
	
	private BroadcastReceiver mDownloadCompleteReceiver;
	
	private DownloadAdmin(Context context) {
		mContext = context;
		mDownloadManager = (DownloadManager) mContext.getSystemService(Context.DOWNLOAD_SERVICE);
	}
	
	public static DownloadAdmin getInstance(Context context) {
		if (sInstance == null) {
			sInstance = new DownloadAdmin(context.getApplicationContext());
		}
		return sInstance;
	}
